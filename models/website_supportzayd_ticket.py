...
class WebsiteSupportTicket(models.Model):
    ...
    def unlink(self):
        ...
        for record in self:
            ...
            record.env.cr.execute(
                "DELETE FROM mail_message WHERE model='website.supportzayd.ticket' AND res_id = %s" % (record_id))
    ...
    def _read_group_state(self, states, domain, order):
        ...
        """
        staff_replied_state = self.env.ref(
            'website_supportzayd.website_ticket_state_staff_replied')
        customer_replied_state = self.env.ref(
            'website_supportzayd.website_ticket_state_customer_replied')
        customer_closed = self.env.ref(
            'website_supportzayd.website_ticket_state_customer_closed')
        staff_closed = self.env.ref(
            'website_supportzayd.website_ticket_state_staff_closed')
    ...
    def _default_state(self):
        return self.env.ref('website_supportzayd.website_ticket_state_open')

    def _default_approval_id(self):
        try:
            return self.env.ref('website_supportzayd.no_approval_required')
        except ValueError:
            return False

    equipment_location = fields.Char(string='Equipment Location', readonly=True, force_save="1",
                                     placeholder="Eg: Bahagian Kewangan, Aras 5")
    channel = fields.Char(string="Channel", default="Manual")
    create_user_id = fields.Many2one('res.users', "Create User")
    priority_star = fields.Selection(AVAILABLE_PRIORITIES, index=True, ondelete='set default')
    partner_id = fields.Many2one('res.partner', string="Customer")
    partner_image = fields.Binary(related='partner_id.image_128', string="Partner image", readonly='True')
    ...
    category = fields.Many2one('website.supportzayd.ticket.categories', required="True", string="Category",
                               track_visibilty='onchange')
    sub_category_id = fields.Many2one('website.supportzayd.ticket.subcategory', string="Sub Category")
    problem = fields.Many2many('problem_many2many_tags',
                               string="Problem")  # TODO this should be renamed to problem_ids because its a many2many field. but currently have no time to change too at email templates, and views
    subject = fields.Char(string="Subject")
    description = fields.Text(string="Description")
    state = fields.Many2one('website.supportzayd.ticket.states', group_expand='_read_group_state',
                            default=_default_state, string="Status")
    state_id = fields.Integer(related='state.id', string="State ID")
    conversation_history = fields.One2many('website.supportzayd.ticket.message', 'ticket_id',
                                           string="Conversation History")
    attachment = fields.Binary(string="Attachments")
    attachment_filename = fields.Char(string="Attachment Filename")
    attachment_ids = fields.One2many('ir.attachment', 'res_id',
                                     domain=[('res_model', '=', 'website.supportzayd.ticket')],
    ...
    contact_num = fields.Char(string="Contact Number",
                              help="by default will prioritize taking customer's mobile number and then phone number.")
    ...
    approval_id = fields.Many2one('website.supportzayd.ticket.approval', default=_default_approval_id,
                                  string="Approval")
    ...
    sla_duration_helpdesk = fields.Float(string="Duration of Helpdesk Response Time: ",
                                         help="this is optional field, it is set only if helpdesk response time exist in SLA",
                                         compute="_compute_sla_calculation", default="0.0000000", store=True)
    sla_duration_onsite = fields.Float(string="Duration of On-Site Response Time: ",
                                       help="this is optional field, it is set only if onsite response time exist in SLA. Onsite response time means how long it takes for technician to get to customer's location",
                                       compute="_compute_sla_calculation", default="0.0000000", store=True)
    sla_duration_resolution = fields.Float(string="Duration of Resolution Response Time: ",
                                           help="This is resolution response time in SLA",
                                           compute="_compute_sla_calculation", default="0.0000000", store=True)
    # hafizalwi 20 jun creating time exceeding SLA
    sla_timer_violation_helpdesk = fields.Float(string="Time exceeding for Helpdesk SLA",
                                                help="SLA's violation for helpdesk", compute="_compute_sla_calculation",
                                                store=True)
    sla_timer_violation_onsite = fields.Float(string="Time exceeding for Onsite SLA", help="SLA's violation for onsite",
                                              compute="_compute_sla_calculation", store=True)
    sla_timer_violation_resolution = fields.Float(string="Time exceeding for Resolution SLA",
                                                  help="SLA's violation for resolution",
                                                  compute="_compute_sla_calculation", store=True)
    ...
    sla_duration_helpdesk_start = fields.Datetime(string="Helpdesk Response Time Start: ",
                                                  compute="_compute_sla_calculation", store=True)
    sla_duration_onsite_start = fields.Datetime(string="Onsite Response Time Start: ",
                                                compute="_compute_sla_calculation", store=True)
    sla_duration_resolution_start = fields.Datetime(string="Resolution Response Time Start: ",
                                                    compute="_compute_sla_calculation", store=True)
    ...
    def _compute_sla_calculation(self):
        for record in self:
            record.sla_timer = record.sla_policies_id.SLA_tot_allocated  # set sla timer to total allocated first, then minus later in the conditional ifs

            if record.sla_policies_id.SLA_helpdesk_response_time > 0.0 and record.create_date and record.open_case:
                helpdesk_timer = record.find_difference_in_time_YminusX(record.open_case, record.create_date)
                record.sla_duration_helpdesk = helpdesk_timer
                record.sla_duration_helpdesk_start = record.open_case

                if 0.0 < record.sla_duration_helpdesk <= record.sla_policies_id.SLA_helpdesk_response_time:  # let say helpdesk response duration does not exist sla_heldpesk_Reponse_time in sla_policies_id
                    # record.sla_timer = record.sla_timer - record.sla_policies_id.SLA_helpdesk_response_time
                    # check if this(helpdesk) is the last point in sla calculation, by checking for future point exist or not
                    if not record.sla_policies_id.SLA_onsite_response_time > 0.0 and not record.sla_policies_id.SLA_resolution_response_time > 0.0:
                        record.sla_timer = record.sla_timer - record.sla_duration_helpdesk
                    else:
                        record.sla_timer = record.sla_timer - record.sla_policies_id.SLA_helpdesk_response_time

                elif record.sla_duration_helpdesk > 0.0 and record.sla_duration_helpdesk > record.sla_policies_id.SLA_helpdesk_response_time:  # if the helpdesk response duration exceed the allocated given time
                    record.sla_timer_violation_helpdesk = record.sla_policies_id.SLA_helpdesk_response_time - record.sla_duration_helpdesk
                    record.sla_timer = record.sla_timer - record.sla_duration_helpdesk

            if record.sla_policies_id.SLA_onsite_response_time > 0.0 and record.create_date and record.check_in:
                hours = record.find_difference_in_time_YminusX(record.create_date, record.check_in)
                record.sla_duration_onsite = hours  # do casting(float) on 'diff' variable
                record.sla_duration_onsite_start = record.create_date
                if 0.0 < record.sla_duration_onsite <= record.sla_policies_id.SLA_onsite_response_time:
                    # record.sla_timer = record.sla_timer - record.sla_policies_id.SLA_onsite_response_time
                    if not record.sla_policies_id.SLA_resolution_response_time > 0.0:
                        record.sla_timer = record.sla_timer - record.sla_duration_onsite
                    else:
                        record.sla_timer = record.sla_timer - record.sla_policies_id.SLA_onsite_response_time
                elif record.sla_duration_onsite > 0.0 and record.sla_duration_onsite > record.sla_policies_id.SLA_onsite_response_time:  # if the helpdesk response duration exceed the allocated given time
                    record.sla_timer_violation_onsite = record.sla_policies_id.SLA_onsite_response_time - record.sla_duration_onsite
                    record.sla_timer = record.sla_timer - record.sla_duration_onsite
            ...
            if record.sla_policies_id.SLA_resolution_response_time > 0.0 and record.check_in and record.check_out:
                hours = record.find_difference_in_time_YminusX(record.check_in, record.check_out)
                record.sla_duration_resolution = hours  # do casting(float) on 'diff' variable
                record.sla_duration_resolution_start = record.check_in
                if 0.0 < record.sla_duration_resolution <= record.sla_policies_id.SLA_resolution_response_time:
                    ...
                    record.sla_timer = record.sla_timer - record.sla_duration_resolution

                elif record.sla_duration_resolution > 0.0 and record.sla_duration_resolution > record.sla_policies_id.SLA_resolution_response_time:  # if the helpdesk response duration exceed the allocated given time
                    record.sla_timer_violation_resolution = record.sla_policies_id.SLA_resolution_response_time - record.sla_duration_resolution
                    record.sla_timer = record.sla_timer - record.sla_duration_resolution
    ...
    item = fields.Many2one('website.supportzayd.ticket.item', string='Equipment (Serial Number)',
                           help="To search, use serial number as referrence")
    sla_alert_ids = fields.Many2many('website.supportzayd.sla.alert', string="SLA Alerts",
    ...
    compute_field = fields.Boolean(string="check field",
                                   compute='_get_user')  # This is a compute variable to filter data according to groups_id @hafizalwi16nov
    ...
    def get_closedticket_today_count(self):
        ...
        for record in self.env['website.supportzayd.ticket'].search([]):
            if record.close_time:
                date1 = datetime.datetime.strptime(record.close_time,
                                                   DEFAULT_SERVER_DATETIME_FORMAT)  # need to convert "record.create_date" which is in str format in python, into datetime to be able to make conditional if
                date2 = datetime.datetime.today()
                new_date2 = date2.replace(hour=0, minute=0,
                                          second=0)  # change it so that the comparison will start from today at oo:oo which means midnight
                if record.state.name in ['Customer Closed',
                                         'Staff Closed'] and date1 > new_date2:  # check later if this code includes the date from yesterday\
    ...
    def get_closedticket_week_count(self):
        ...
        for record in self.env['website.supportzayd.ticket'].search([]):
            if record.close_time:
                ...
                new_date2 = date2.replace(hour=23, minute=59,
                                          second=59)  # convert datetime to today at end of day (11:59pm), and then minus 7 days from the date, eg: it will show data from last sunday to this saturday
                if record.state.name in ['Customer Closed', 'Staff Closed'] and date1 > (new_date2 - timedelta(
                        days=7)):  # according to this logic, it will start from 12am last 7 days, notice the operator show > means not including 23:59pm from last week
                    total_closed = total_closed + 1
    ...
    def get_closedticket_month_count(self):
        ...
        for record in self.env['website.supportzayd.ticket'].search([]):
            if record.close_time:
                datetime1 = datetime.datetime.strptime(record.close_time,
                                                       DEFAULT_SERVER_DATETIME_FORMAT)  # need to convert "record.create_date" which is in str format in python, into datetime to be able to make conditional if
                ...
                if record.state.name in ['Customer Closed',
                                         'Staff Closed'] and date1 <= last_day_of_month and date1 >= first_day_of_month:  # according to this logic, it will start from 12am last 7 days, notice the operator show > means not including 23:59pm from last week
                    total_closed = total_closed + 1
        return total_closed
    ...
    def get_openticket_count(self):
        total_open = 0
        for record in self.env['website.supportzayd.ticket'].search([]):
            if record.state.name in ['Open',
                                     'Customer Replied']:  # later ask Helpdesk if they want the open to include other state such as "awaiting approval", "approval accepted", etc except "closed" state.
    ...
    def get_priority_star_string(self):
        ...
        for record in self:
            string_priority = dict(record._fields['priority_star'].selection).get(record.priority_star)
    ...
    def get_failedSLA_highpriority_count(self):
        ...
        for record in self.env['website.supportzayd.ticket'].search([]):
            if record.state.name in ['Open',
                                     'Customer Replied'] and record.sla_timer < 0 and record.priority_star == '2':  # if the sla_timer is finished and priority is 4(High Prio), then count it as failed SLA with high priority ticket
                total_failed = total_failed + 1
    ...
    def get_failedSLA_urgent_count(self):
        ...
        for record in self.env['website.supportzayd.ticket'].search([]):
            if record.state.name in ['Open',
                                     'Customer Replied'] and record.sla_timer < 0 and record.priority_star == '3':  # if the sla_timer is finished and priority is 4(High Prio), then count it as failed SLA with high priority ticket
                total_failed = total_failed + 1
        return total_failed
    ...
    def get_success_rate_today(self):
        ...
        for record in self.env['website.supportzayd.ticket'].search([]):
            if record.close_time:
                ...
                if record.state.name in ['Staff Closed',
                                         'Customer Closed'] and date1 >= start_date and date1 <= end_date:
                    countClosed += 1
    ...
    def get_success_rate_week(self):
        ...
        for record in self.env['website.supportzayd.ticket'].search([]):
            if record.close_time:
                ...
                if record.state.name in ['Staff Closed', 'Customer Closed'] and date1 >= (
    ...
    #TODO 14/2/2023 commented this whole function def_onchange_category because its not function
    ...
    @api.onchange(
        'sla_active')  # this means if we literally clicks on the sla_active (not pause resume sla), because pause resume sla can check or not check the checkbox too
    ...
    def _onchange_sla_active(self):
        ...
        if self.open_case and self.sla_policies_id and self.sla_active is True and self.sla_timer == 0.0:
            ...
            if not self.open_case and self.sla_active:
                ...
                self.sla_active = False
                return {'value': {}, 'warning': {'title': 'Alert',
                                                 'message': 'The selected SLA ID have SLA response time. To continue, please create the ticket first!'}}
    ...
    def _compute_ticket_number_display(self):
        ...
        for record in self:
            if record.ticket_number:
                record.ticket_number_display = str("TN") + str(record.id) + "-" + datetime.datetime.today().strftime(
                    '%Y%m%d') + "-" + "{:,}".format(record.ticket_number)
            else:
                record.ticket_number_display = record.id
    ...
    def _compute_sla_timer_format(self):
        for record in self:
            ...
            record.sla_timer_format = '{0:02.0f}:{1:02.0f}'.format(*divmod(abs(record.sla_timer) * 60, 60))
    ...
    @api.model
    @api.depends('planned_time', 'partner_id.lang', 'partner_id.tz')
    def _compute_planned_time_format(self):
        for record in self:
            if record.planned_time and record.partner_id and record.partner_id.lang:
                partner_language = record.env['res.lang'].search([('code', '=', record.partner_id.lang)], limit=1)
                if partner_language:
                    my_planned_time = datetime.datetime.strptime(record.planned_time, DEFAULT_SERVER_DATETIME_FORMAT)

                    if record.partner_id.tz:
                        my_planned_time = my_planned_time.replace(tzinfo=tz.gettz('UTC'))
                        local_time = my_planned_time.astimezone(tz.gettz(record.partner_id.tz))
                        record.planned_time_format = local_time.strftime(
                            partner_language.date_format + " " + partner_language.time_format) + " " + record.partner_id.tz
                    else:
                        record.planned_time_format = my_planned_time.strftime(
                            partner_language.date_format + " " + partner_language.time_format) + " UTC"
                else:
                    record.planned_time_format = record.planned_time
            else:
                record.planned_time_format = record.planned_time

    def _compute_approve_url(self):
        for record in self:
            record.approve_url = "/supportzayd/approve/" + str(record.id)

    def _compute_disapprove_url(self):
        for record in self:
            record.disapprove_url = "/supportzayd/disapprove/" + str(record.id)
    ...
    def _compute_unattend(self):
        opened_state = self.env.ref('website_supportzayd.website_ticket_state_open')
        customer_replied_state = self.env.ref(
            'website_supportzayd.website_ticket_state_customer_replied')

        for record in self:
            if record.state == opened_state or record.state == customer_replied_state or record.state.unattended == True:
                record.unattended = True

    @api.model
    def request_approval(self):

        approval_email = self.env.ref('website_supportzayd.support_ticket_approval')
    ...
    def _needaction_domain_get(self):
        open_state = self.env.ref('website_supportzayd.website_ticket_state_open')
        custom_replied_state = self.env.ref(
            'website_supportzayd.website_ticket_state_customer_replied')
    ...
    def _check_auto_sla_policies(self):
        ...
        setting_allow_auto_sla_criteria = self.env['ir.default'].get('website.supportzayd.settings',
                                                                     'allow_auto_sla_criteria')
        ...
        for ticket in self:
            if setting_allow_auto_sla_criteria:
                ...
                dom = ['&', ('minimum_priority', '<=', ticket.priority_star), '&', '|',
                       ('category_id', '=', ticket.category.id), ('category_id', '=', False),
    ...
    def create(self, vals):
        ...
        #TODO 14/2/2023 i commented the code below so that can save in odoo 15.
        #ticket_open_email_template = self.env.ref(
        #    'website_supportzayd.website_ticket_state_open').mail_template_id
        ...
        notification_template = self.env.ref(
            'website_supportzayd.new_support_ticket_category')
        support_ticket_menu = self.env.ref(
            'website_supportzayd.website_supportzayd_ticket_menu')
        support_ticket_action = self.env.ref(
            'website_supportzayd.website_supportzayd_ticket_action')

        # Add them as a follower to the ticket so they are aware of any internal notes
        new_id.message_subscribe(partner_ids=new_id.category.cat_user_ids.ids)
        ...
        for my_user in new_id.category.cat_user_ids:
            ...
            values['body_html'] = values['body_html'].replace("_ticket_url_", "web#id=" + str(
                new_id.id) + "&view_mode=form&model=website.supportzayd.ticket&menu_id=" + str(
    ...
    def write(self, values, context=None):
        ...
        if 'state' in values:
            ...
            if self.state.name in ['Staff Closed',
                                   'Customer Closed']:  # call the def closed_ticket function from model website.supportzayd.ticket.close
        ...
        if 'category' in values:
            change_category_email = self.env.ref(
                'website_supportzayd.new_support_ticket_category_change')
        ...
        if 'user_id' in values:
            ...
            if setting_change_user_email_template_id:
                ...
                email_template = self.env.ref(
                    'website_supportzayd.support_ticket_user_change')
    ...
    def send_survey(self):
        notification_template = self.env.ref(
            'website_supportzayd.support_ticket_survey')
    ...
    def _get_user(self):
        for record in self:
            res_user = record.env['res.users'].search([('id', '=', self._uid)])
            if res_user.has_group('website_supportzayd.support_technician') and not res_user.has_group(
                    'sale.group_sale_salesman_all_leads'):
                record.compute_field = True
            else:
                record.compute_field = False


class WebsiteSupportTicketEquipment(
    models.Model):  # The purpose of creating this is to create dropdown suggestion when inserting data for  Equipment name
    _name = "website.supportzayd.ticket.equipment"

    name = fields.Char(string="Name",
                       placeholder="Eg: Acer Aspire 2470")  # this is to store model char ?? @hafizalwi1dec2021
...
class WebsiteSupportTicketMerge(models.TransientModel):
    ...
    def merge_tickets(self):
        ...
        if setting_ticket_merge_email_template_id:
            ...
            # BACK COMPATABLITY FAIL SAFE
            mail_template = self.env.ref('website_supportzayd.support_ticket_merge')
...
class WebsiteSupportTicketMessage(models.Model):
    ...
    by = fields.Selection([('staff', 'Staff'), ('customer', 'Customer')], string="By", ondelete='set default')
...
class WebsiteSupportTicketCategories(models.Model):
    ...
    def _check_duplicate(self):
        for record in self:
            category_tags = record.env['website.supportzayd.ticket.categories'].search_read(
                [('name', 'in', [record.name])])
...
class WebsiteSupportTicketSubCategories(models.Model):
    ...
    parent_category_id = fields.Many2one('website.supportzayd.ticket.categories', required=True,
                                         string="Category")  # default=lambda self: self.env['website.supportzayd.ticket'].search(['category.id','=',self.sub_category_id.id]).category.id) #default=lambda self: self.env['website.supportzayd.ticket'].search(['category.id','=',self.category.id]).category.id)

    @api.model
    @api.constrains('name', 'parent_category_id', )
    def _check_duplicate(self):
        for record in self:
            subcategory_tags = record.env['website.supportzayd.ticket.subcategory'].search_read(
                [('name', 'in', [record.name]), ('parent_category_id', '=', record.parent_category_id.id)]) #TODO 14/2/2023
...
class WebsiteSupportTicketProblem(models.Model):
    ...
    parent_subcategory_id = fields.Many2one('website.supportzayd.ticket.subcategory', required=True,
    ...
    def _check_duplicate(self):
        for record in self:
            problem_tags = record.env['problem_many2many_tags'].search_read(
...
class WebsiteSupportTicketItem(models.Model):
    ...
    equipment_type = fields.Many2one('website.supportzayd.ticket.item.type', string="Type of Equipment",
                                     help="eg: Personal Computer, Printer")
    ...
    customer = fields.Many2one('res.partner', required=True,
                               string='Customer')  # changed String from 'Company location' to 'Customer' 13dec
    user = fields.Char(string='Equipment User')  # changed String from 'Company location' to 'Customer' 13dec

    location_id = fields.Many2one('website.supportzayd.ticket.item.location', string="Current Location",
                                  placeholder="Bahagian Perolehan")
    ...
    partner_reference = fields.Many2many('website.supportzayd.tags', string="Tender Reference",
                                         help="Tender's unique initial code, eg:'DAAS1'", placeholder="Eg:DAAS1")
    ...
    def write(self, values):
        ...
        if 'location_id' in values:
            ...
            self.write({"location_history": [[0, 0, {'location_id': location_id, 'date': date,
                                                     'item_id': self.id}]]})  # notice that in def create for this model i dont do this[0,0,vals], this brings the equivalent meaning like normal logic but this is a special command syntax for odoo
...
class WebsiteSupportTicketClose(models.TransientModel):
    ...
    def close_ticket(self):  # @hafiz15dec
        ...
        email_wrapper = self.env.ref(
            'website_supportzayd.support_ticket_close_wrapper')  # @hafizalwi17dec tested to put 'sudo()' so that data can be imported in mail template without having to give read permission to for certain fields such as 'problem' to group 'Technician', which is not good for security
    ...
    def send_reply(self):
        ...
        for record in self:
            ...
            if record.approval:
                ...
                awaiting_approval_state = record.env.ref(
                    'website_supportzayd.website_ticket_state_awaiting_approval')
                record.ticket_id.state = awaiting_approval_state.id

                # One support request per ticket...
                record.ticket_id.planned_time = record.planned_time
                record.ticket_id.approval_message = record.body
                record.ticket_id.sla_active = False

            # Send email
            values = {}

            setting_staff_reply_email_template_id = record.env['ir.default'].get('website.supportzayd.settings',
                                                                                 'staff_reply_email_template_id')

            if setting_staff_reply_email_template_id:
                email_wrapper = record.env['mail.template'].browse(setting_staff_reply_email_template_id)

            values = email_wrapper.generate_email([record.id])[record.id]
            values['model'] = "website.supportzayd.ticket"
            values['res_id'] = record.ticket_id.id
            values['reply_to'] = email_wrapper.reply_to

            if record.email_cc:
                values['email_cc'] = record.email_cc

            for attachment in record.attachment_ids:
                values['attachment_ids'].append((4, attachment.id))

            send_mail = record.env['mail.mail'].create(values)
            send_mail.send()

            # Add to the message history to keep the data clean from the rest HTML
            record.env['website.supportzayd.ticket.message'].create({'ticket_id': record.ticket_id.id, 'by': 'staff',
                                                                     'content': record.body.replace("<p>", "").replace(
                                                                         "</p>", "")})

            # Post in message history
            # record.ticket_id.message_post(body=record.body, subject=record.subject, message_type='comment', subtype='mt_comment')

            if record.approval:
                # Also change the approval
                awaiting_approval = record.env.ref(
                    'website_supportzayd.awaiting_approval')
                record.ticket_id.approval_id = awaiting_approval.id
            else:
                # Change the ticket state to staff replied
                staff_replied = record.env.ref(
                    'website_supportzayd.website_ticket_state_staff_replied')
                record.ticket_id.state = staff_replied.id
