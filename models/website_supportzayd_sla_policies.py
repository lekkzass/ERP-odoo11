...
class WebsiteSupportSLAPolicies(models.Model):
    ...
    category_id = fields.Many2one('website.supportzayd.ticket.categories', string="Category",
                                  help="Select which category to apply SLA,leave empty to apply to all categories")
    minimum_priority = fields.Selection(AVAILABLE_PRIORITIES, index=True, ondelete='set default')
    partner_ids = fields.Many2many('res.partner', string="Customers",
                                   help="select applicable for which customers, leave empty to select all customers")
    ...
    countdown_condition = fields.Selection([('business_only', 'Business Only'), ('24_hour', '24 Hours')],
                                           default="24_hour", required="True", ondelete='set default')

    SLA_helpdesk_response_time = fields.Float(string="SLA Helpdesk response time: ",
                                              help="Onsite timer is duration from open case to create date")
    SLA_onsite_response_time = fields.Float(string="SLA On-Site response time: ",
                                            help="Onsite timer is duration from create date to checkin")
    SLA_resolution_response_time = fields.Float(string="SLA Problem resolution response time: ",
                                                help="Resolution timer is duration from checkin to checkout")
    SLA_tot_allocated = fields.Float(string='SLA Total Allocated Time',
                                     compute="get_total_allocated_SLA")  # note this is the total of SLA1 aka helpdeskresponse(if exist) + SLA2 aka onsiteResponse + SLA3 aka problem resoluton time
    countdown_condition = fields.Selection([('business_only', 'Business Only'), ('24_hour', '24 Hours')],
                                           default="24_hour", required="True", ondelete='set default')

    # alert_ids = fields.One2many('website.supportzayd.sla.policies', 'vsa_id', string="Email Alerts"
    @api.model
    @api.depends('SLA_resolution_response_time', 'SLA_helpdesk_response_time', 'SLA_onsite_response_time')
    def get_total_allocated_SLA(self):
        for record in self:
            total = record.SLA_resolution_response_time
            if record.SLA_helpdesk_response_time:
                total += record.SLA_helpdesk_response_time

            if record.SLA_onsite_response_time:
                total += record.SLA_onsite_response_time
            record.SLA_tot_allocated = total
